export const services = [
  {
    id: 'web-development',
    title: 'Web Development',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Neque repellendus minima reiciendis nobis dolore obcaecati.',
  },
  {
    id: 'digital-marketing',
    title: 'Digital Marketing',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Neque repellendus minima reiciendis nobis dolore obcaecati.',
  },
  {
    id: 'saas-products',
    title: 'SaaS Products',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Neque repellendus minima reiciendis nobis dolore obcaecati.',
  },
  {
    id: 'apps-development',
    title: 'Apps Development',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Neque repellendus minima reiciendis nobis dolore obcaecati.',
  },
  {
    id: 'seo-services',
    title: 'SEO Services',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Neque repellendus minima reiciendis nobis dolore obcaecati.',
  },
  {
    id: 'data-analysis',
    title: 'Data Analysis',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Neque repellendus minima reiciendis nobis dolore obcaecati.',
  },
];

export const speakersData = [
  { img: '/images/speakers/img-1.webp', alt: 'girl working on a computer' },
  { img: '/images/speakers/img-2.webp', alt: 'business woman posing street' },
  {
    img: '/images/speakers/img-3.webp',
    alt: 'beautifull caucasian woman with curly hair smiles isolated',
  },
];

export const casesData = [
  { img: '/images/cases/img-1.webp', alt: 'slide cases 1' },
  { img: '/images/cases/img-2.webp', alt: 'slide cases 2' },
  {
    img: '/images/cases/img-3.webp',
    alt: 'slide cases 3',
  },
  {
    img: '/images/cases/img-4.webp',
    alt: 'slide cases 4',
  },
  {
    img: '/images/cases/img-5.webp',
    alt: 'slide cases 5',
  },
  {
    img: '/images/cases/img-6.webp',
    alt: 'slide cases 6',
  },
  {
    img: '/images/cases/img-7.webp',
    alt: 'slide cases 7',
  },
  {
    img: '/images/cases/img-8.webp',
    alt: 'slide cases 8',
  },
  {
    img: '/images/cases/img-9.webp',
    alt: 'slide cases 9',
  },
  {
    img: '/images/cases/img-10.webp',
    alt: 'slide cases 10',
  },
  {
    img: '/images/cases/img-11.webp',
    alt: 'slide cases 11',
  },
  {
    img: '/images/cases/img-12.webp',
    alt: 'slide cases 12',
  },
];
